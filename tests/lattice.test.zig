...
const createLattice = core.lattice.createLattice;
...
test "lattice: create lattice" {
    ...
    var lattice = try createLattice(allocator, "こんにちはせかい", &dict);
    defer lattice.deinit();

    // Test the structure of the lattice
    try testing.expectEqual(@as(usize, 10), lattice.nodes.items.len);
    // Calculate total number of nodes
    var total_nodes: usize = 0;
    for (lattice.nodes.items) |level| {
        total_nodes += level.items.len;
    }
    try testing.expectEqual(@as(usize, 180), total_nodes);
}

test "lattice: find correct path" {
    const allocator = testing.allocator;
    ...
    const path = try lattice.findBestPath(&dict);
